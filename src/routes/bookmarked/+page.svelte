<script lang="ts">
  ...
  onMount(() => {
    ...
    englishBookmarkItems = filteredBookmarkItems.filter(
      (item) => item.translateLanguage === "English"
    );
    spanishBookmarkItems = filteredBookmarkItems.filter(
      (item) => item.translateLanguage === "Spanish"
    );
  ...
  const applyFilter = () => {
    ...
    if (selectedTagsContext.length > 0) {
      selectedTagsContext.forEach((context) => {
        filteredBookmarkItems = filteredBookmarkItems.filter(
    ...
    if (selectedRegion.length > 0) {
      selectedRegion.forEach((region) => {
        filteredBookmarkItems = filteredBookmarkItems.filter(
          (item) => item.region === region.text
        );
      });
    }
    if (selectedLanguage) {
      filteredBookmarkItems = filteredBookmarkItems.filter(
        (item) => item.translateLanguage === selectedLanguage
      );
    }
    englishBookmarkItems = filteredBookmarkItems.filter(
      (item) => item.translateLanguage === "English"
      ...
      {#if englishBookmarkItems.length > 0}
        ...
        <div class="bookmarks-container">
          {#each englishBookmarkItems as bookmarkItem}
            <BookmarkedItem
              phrase={bookmarkItem.phrase}
              originLanguage={bookmarkItem.originLanguage}
              translateLanguage={bookmarkItem.translateLanguage}
              region={bookmarkItem.region}
              context={bookmarkItem.context}
              partSpeech={bookmarkItem.partSpeech}
              phoneticSpelling={bookmarkItem.phoneticSpelling}
              examples={bookmarkItem.examples}
    ...
    <form action="">
      <div class="spacer">
        <div class="language-filters">
          <span class="language-text"> Please select a language first </span>
          <fieldset class="flex">
