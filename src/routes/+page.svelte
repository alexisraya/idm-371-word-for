    ...
    <form on:submit|preventDefault={handleSubmit}>
      <div class="dropdown-container">
        <div class="dropdown-language-container">
          <select
            ...
            on:change={handleOriginLanguageChange}
          >
          ...
          <select
            class="dropdown-thin"
            bind:value={selectedTranslate}
            on:change={handleTranslateLanguageChange}
          >
            {#each languages as language}
              <option value={language}>
                {language}
      ...
      <div class="translate-text-field-container">
        ...
        <button class="record-button" type="button" on:click={handleAudio}>
          {#if isRecording}
            <img
              class="record-icon-active"
              src={microphoneActive}
              alt="record-icon"
            />
          {:else}
            <img
              class="record-icon-inactive"
              src={microphone}
              alt="record-icon"
            />
          {/if}
...
<style>
  ...
  .record-icon-inactive {
    animation: 0.225s cubic-bezier(0.51, 0.92, 0.24, 1.15) 0s 1 scaleImage;
  }
